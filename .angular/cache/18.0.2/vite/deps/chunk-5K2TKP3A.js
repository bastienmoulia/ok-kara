...
function isActionKeyEvent(e) {
  ...
  const isSpecificTriggerKey =
    IDENTIFIER_TO_KEY_TRIGGER_MAPPING[type] % key === 0;
  const isDefaultTriggerKey =
    !(type in IDENTIFIER_TO_KEY_TRIGGER_MAPPING) && key === KeyCode.ENTER;
...
var ActionResolver = class {
  ...
  addA11yClickSupport(
    updateEventInfoForA11yClick2,
    preventDefaultForA11yClick2,
    populateClickOnlyAction2,
  ) {
...
function populateClickOnlyAction(actionElement, eventInfo, actionMap) {
  if (
    ...
    getA11yClickKey(eventInfo) ||
    actionMap[EventType.CLICKONLY] === void 0
  ) {
    return;
  }
...
var _EventContract = class _EventContract {
  ...
  addEvent(eventType, prefixedEventType) {
    ...
    if (
      ...
      NON_BUBBLING_MOUSE_EVENTS.indexOf(eventType) >= 0
    ) {
      return;
    }
...
function trySerializeI18nBlock(lView, index, context) {
  ...
  const caseQueue = [];
  tI18n.ast.forEach((node) =>
    serializeI18nBlock(lView, caseQueue, context, node),
  );
  ...
  switch (node.kind) {
    ...
    case 2:
      node.children.forEach((node2) =>
        serializeI18nBlock(lView, caseQueue, context, node2),
      );
      break;
    case 3:
      ...
      if (currentCase != null) {
        ...
        node.cases[caseIdx].forEach((node2) =>
          serializeI18nBlock(lView, caseQueue, context, node2),
        );
      }
      break;
...
function collectI18nNodesFromDom(context, state, nodeOrNodes) {
  if (Array.isArray(nodeOrNodes)) {
    for (const node of nodeOrNodes) {
      ...
      const nextState = targetNode
        ? forkHydrationState(state, targetNode)
        : state;
  ...
  bootstrapModuleFactory(moduleFactory, options) {
    ...
    return ngZone.run(() => {
      ...
      if (typeof ngDevMode === 'undefined' || ngDevMode) {
        ...
        if (
          moduleRef.injector.get(ZONELESS_ENABLED) &&
          options?.ngZone !== 'noop'
        ) {
          throw new RuntimeError(
            408,
            "Invalid change detection configuration: `ngZone: 'noop'` must be set in `BootstrapOptions` with provideExperimentalZonelessChangeDetection.",
          );
        }
      }
